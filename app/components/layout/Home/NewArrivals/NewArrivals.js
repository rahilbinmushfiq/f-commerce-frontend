...
const NewArrivals = () => {
  const newlyArrivedProducts = products
    .filter((product) => product.isNewArrival && product.inStock)
    .slice(0, 4);
  ...
  if (newlyArrivedProducts.length)
    return (
      <div className="relative w-full overflow-hidden bg-[#FBEDE2]">
        <div className="absolute inset-0 ml-auto bg-[url('/shapes/curved-dotted-line-2.svg')] bg-right-top bg-no-repeat opacity-50 max-lg:top-1/2 max-lg:-translate-y-7 max-lg:bg-contain lg:max-lg:w-2/3" />
        <div className="relative mx-auto px-5 py-8 md:py-10 lg:py-12 xl:max-w-[1200px] xl:px-0">
          ...
          <section className="grid grid-cols-2 gap-x-4 gap-y-12 sm:grid-cols-3 lg:grid-cols-4 sm:max-lg:[&>div:last-child]:hidden">
            {newlyArrivedProducts.map(
              (newlyArrivedProduct, newlyArrivedProductIndex) => {
                return (
                  <div
                    key={newlyArrivedProductIndex + newlyArrivedProduct.title}
                  ...
                  >
                    ...
                    >
                      <div className="relative mb-3 h-[250px] w-full overflow-hidden rounded-[20px] bg-[#F0F0F0] sm:h-80 lg:h-[26vh]">
                        ...
                        <div className="absolute left-4 top-4 space-y-2">
                          {newlyArrivedProduct.salesThisMonth >= 15 && (
                            <div className="relative h-9 w-9 overflow-hidden rounded-lg bg-[#cd4747] font-semibold text-white shadow-[1px_1px_12px_0_rgba(0,0,0,0.1)] transition-[background-color,width] hover:w-[calc(36px+58px+10px)]">
                              ...
                              <span className="absolute left-9 top-1/2 -translate-y-1/2 text-nowrap text-[13px]">
                                Trending!
                              </span>
                            </div>
                          )}
                          {newlyArrivedProduct.isNewArrival && (
                          ...
                          {newlyArrivedProduct.discount && (
                            <div
                              className={`relative h-9 w-9 overflow-hidden rounded-lg bg-[#32aa54] font-semibold text-white shadow-[1px_1px_12px_0_rgba(0,0,0,0.1)] transition-[background-color,width] ${newlyArrivedProduct.discount.amount.includes("%") ? "hover:w-[calc(36px+56px+10px)]" : "hover:w-[calc(36px+68px+10px)]"}`}
                            >
                              <TbRosetteDiscount className="mx-1.5 h-9 w-6 object-contain" />
                              <span className="absolute left-9 top-1/2 -translate-y-1/2 text-nowrap text-[13px]">
                                {newlyArrivedProduct.discount.amount} OFF!
                              </span>
                            </div>
                          )}
                        </div>
                        {!newlyArrivedProduct.inStock && (
                      ...
                      <div className="flex gap-x-3 [&_p]:w-fit [&_p]:bg-[#FBEDE2]">
                        <p
                          className={`relative w-fit text-base font-semibold xl:text-lg ${newlyArrivedProduct.discount ? "text-neutral-400 before:absolute before:left-0 before:right-0 before:top-1/2 before:h-0.5 before:w-full before:-translate-y-1/2 before:bg-neutral-400 before:content-['']" : "text-neutral-800"}`}
                        >
                          ৳ {newlyArrivedProduct.price.toLocaleString()}
                        </p>
                        {newlyArrivedProduct.discount && (
                          <p className="text-base font-semibold text-neutral-800 xl:text-lg">
                            ৳{" "}
                            {newlyArrivedProduct.discount.finalPrice.toLocaleString()}
                          </p>
                        )}
                      </div>
                      <h3 className="line-clamp-1 w-fit bg-[#FBEDE2] text-base font-semibold text-neutral-800 xl:text-lg">
                        {newlyArrivedProduct.title}
                      </h3>
                      <p className="mt-1.5 line-clamp-1 w-fit bg-[#FBEDE2] text-xs text-neutral-700 xl:text-sm">
                        {newlyArrivedProduct.categories.mainCategory +
                          ", " +
                          newlyArrivedProduct.categories.subCategories.join(
                            ", ",
                          )}
                      </p>
                    </TransitionLink>
                    <div className="absolute right-2.5 top-2.5 flex -translate-x-5 flex-col items-end space-y-1.5 opacity-0 transition-[transform,opacity] duration-300 ease-in-out [&>button]:rounded-lg [&>button]:font-semibold [&>button]:text-neutral-700 [&>button]:shadow-[3px_3px_20px_0_rgba(0,0,0,0.25)] hover:[&>button]:opacity-100">
                      {!!newlyArrivedProduct.inStock && (
                        <button
                          ...
                          onClick={() => {
                            setSelectedAddToCartProduct(newlyArrivedProduct);
                      ...
                      <button
                        ...
                        onClick={() => handleAddToWishlist(newlyArrivedProduct)}
                      >
                    ...
                    <div className="absolute left-1/2 top-[250px] flex w-full -translate-x-1/2 -translate-y-2/3 items-center gap-x-1.5 rounded-b-3xl border border-[#F0F0F0] bg-white bg-opacity-75 px-4 py-3 opacity-0 transition-[transform,opacity] duration-300 ease-in-out sm:top-[320px] sm:gap-x-2.5 lg:top-[26vh]">
                      ...
                      <div className="flex items-center justify-center sm:gap-x-1">
                        {newlyArrivedProduct.colors.map((color, colorIndex) => {
                          return (
                            <div
